...
const VelocityChart = () => {
  ...
  return (
    <div className="bg-white rounded-3xl p-6 shadow-sm border border-gray-100 flex-1 min-w-[500px] flex flex-col">
      {/* Header */}
      ...
      <div className="relative pt-8 pb-4 px-6 bg-blue-50/30 rounded-2xl border border-blue-50/50">
        <div className="flex items-end justify-between gap-6 h-[180px]">
          {bars.map((bar, i) => (
            <div key={i} className="flex-1 group relative flex flex-col items-center h-full justify-end">
              ...
              <div
                className={`w-full max-w-[32px] rounded-t-lg transition-all duration-700 cursor-pointer ${bar.color} hover:brightness-95 shadow-sm shadow-blue-900/5`}
                style={{ height: bar.h }}
              ></div>
            </div>
          ))}
      ...
      {/* X-Axis Labels */}
      <div className="flex justify-between mt-3 text-[12px] font-medium text-gray-400 px-1">
