  ...
  return (
    <div className="bg-gradient-to-br from-white to-[#fcfdff]/90 backdrop-blur-md border border-white/60 rounded-3xl p-5 hover:shadow-[0_8px_24px_-4px_rgba(148,163,184,0.08)] transition-all duration-300 shadow-[0_4px_20px_-4px_rgba(148,163,184,0.06)]">
      ...
      <div className="pb-3.5 border-b border-slate-100/80 mb-4 flex items-center justify-between">
        <div>
          <span className="text-[9px] font-black text-[#704efe] uppercase tracking-widest block">Flow Rates</span>
          <h3 className="text-xs font-black text-slate-800 uppercase tracking-wider mt-0.5">High Velocity Items</h3>
        </div>
        <span className="px-2 py-0.5 rounded-lg bg-cyan-50 border border-cyan-100 text-cyan-600 text-[8.5px] font-black uppercase tracking-wider">
          Fast Movers
        </span>
      </div>
      ...
      <div className="flex flex-col gap-3">
        {suggestions.length === 0 ? (
          <div className="py-6 text-center text-slate-400 text-xs font-semibold uppercase">
            No fast movers detected
          </div>
        ) : (
          suggestions.slice(0, 3).map((item, idx) => {
            const strokeColor = idx === 0 ? '#f43f5e' : idx === 1 ? '#fbbf24' : '#704efe';

            return (
              <div key={item.id} className="flex items-center justify-between gap-3 bg-slate-50/40 p-3 rounded-2xl border border-slate-100 hover:bg-slate-50/70 transition-all shadow-3xs">
                <div className="min-w-0 flex-1">
                  <span className="text-[12px] font-black text-slate-800 truncate block leading-none">{item.name}</span>
                  <span className="text-[9px] text-slate-400 font-mono font-bold uppercase tracking-wider block mt-1.5">{item.sku}</span>
                </div>
